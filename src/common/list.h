...
/* 遍历链表，修改节点可能导致循环出错 */
#define commonListIterate(pos, head) 	for (pos = (head)->next; pos != (head); pos = pos->next)

/* 遍历链表，删除修改节点不会导致循环出错 */
#define commonListIterateSafe(pos, n, head) 	for (pos = (head)->next, n = pos->next; pos != (head); 		pos = n, n = pos->next)
